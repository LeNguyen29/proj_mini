...
public class PredEval
{
  ...
  public static boolean Eval(CondExpr p[], Tuple t1, Tuple t2, AttrType in1[], 
	...
	while (p[i] != null)
	{
	  ...
	  while (temp_ptr != null)
	  {
		// Setup first argument for comparison.
		...
		switch (temp_ptr.type1.attrType)
		{
			...
			case AttrType.attrVector100D:
				value.setHdr((short)1, val_type, null);
				value.set100DVectFld(1, temp_ptr.operand1.vec100Dtype);
				tuple1 = value;
				comparison_type.attrType = AttrType.attrVector100D;

				temp_ptr.distance = 1; // To show distance is being used
				break;
			default:
			break;
		}
		...
		switch (temp_ptr.type2.attrType)
		{
			...
			case AttrType.attrVector100D:
				value.setHdr((short)1, val_type, null);
				value.set100DVectFld(1, temp_ptr.operand2.vec100Dtype);
				tuple2 = value;
				comparison_type.attrType = AttrType.attrVector100D;
		...
		try {
			// If result is > 1, then it's probably distance between tuples
		...
		{
			int distParam = temp_ptr.distance; // PLACE HOLDER, LIKELY NEED TO DO QUERY PART FOR THIS

			// compare computed vector distance with threshold using specified operator
			switch (temp_ptr.op.attrOperator) {
				case AttrOperator.aopGT:
					op_res = (comp_res > distParam);
					break;
